Shader "Skin/NotSoFastNotSoFakeSkin" {
        ...
        SubShader {
			...
			half3 Skin_BRDF_PBS (SurfaceOutputStandardSkin s, float oneMinusReflectivity, half3 viewDir, UnityLight light, UnityIndirect gi)
			{
				...
				color.rgb += 0
							+ s.Albedo * (_AmbientContribution * gi.diffuse + occl * brdf)
            ...
            void surf (Input IN, inout SurfaceOutputStandardSkin o) {
                ...
                half4 c = tex2D (_MainTex, IN.uv_MainTex) * _Color2;
                ...
                half mask = tex2D (_MainTex, IN.uv_MainTex).a;
                ...
                o.Specularity = tex2D (_MaskTex, IN.uv_MaskTex).r * _SpecPow;    
                o.Smoothness = tex2D (_MaskTex, IN.uv_MaskTex).g * _GlossPow;              
                o.Occlusion = tex2D (_MaskTex, IN.uv_MaskTex).b;                                     
            }
            ENDCG
        }
        FallBack "Diffuse"
